...
if TYPE_CHECKING:
    ...
    from plugin_factory.state_machine import PluginState
...
class PluginStateManager:
    """for the future"""
    def __init__(self, allow_state: TransitionInterface) -> None:
        self.__allow_state = allow_state
        self.__plugins: dict[str, PluginBase] = {}

        self.__state_transitions = StateTransition(self.__allow_state)
...
# region method add plugin
    def add_plugins(self, plugins: Mapping[str, PluginBase]) -> None:
        logger.debug("Adding plugins (checking duplicate): %s", plugins)
        for plugin_name in plugins.keys():
            if self.__is_plugin_exist(plugin_name):
                logger.warning("Plugin %s already exists", plugin_name)
                return
    ...
    def init_all_plugin(self) -> None:
        ...
        for plugin in self.__plugins.values():
            try:
                ...
                logger.error("Failed to initialize plugin %s: %s", plugin.info.name, exc)
    ...
    def start_all_plugin(self) ->  None:
        ...
        for plugin in self.__plugins.values():
            try:
                ...
                logger.error("Failed to started plugin %s: %s", plugin.info.name, exc)
    ...
    def stop_all_plugin(self) -> None:
        ...
        for plugin in self.__plugins.values():
            try:
                ...
                logger.error("Failed to stopped plugin %s: %s", plugin.info.name, exc)
# endregion

# region method for one plugin
